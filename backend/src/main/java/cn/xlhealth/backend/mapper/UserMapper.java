...
public interface UserMapper extends BaseMapper<User> {
    ...
    int updateLastLoginInfo(@Param("userId") Long userId, @Param("lastLoginTime") LocalDateTime lastLoginTime, @Param("lastLoginIp") String lastLoginIp);
    ...
    List<User> findByRegistrationTimeRange(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);
    ...
    /**
     * 分页查询用户列表（支持关键词搜索和状态筛选）
     */
    @Select("SELECT * FROM users WHERE (#{keyword} IS NULL OR #{keyword} = '' OR username LIKE CONCAT('%', #{keyword}, '%') OR email LIKE CONCAT('%', #{keyword}, '%') OR nickname LIKE CONCAT('%', #{keyword}, '%')) AND (#{status} IS NULL OR status = #{status}) ORDER BY created_time DESC")
    IPage<User> selectUserPage(Page<User> page, @Param("keyword") String keyword, @Param("status") String status);
     ...
     * 更新用户状态
     */
    @Update("UPDATE users SET status = #{status}, updated_time = NOW() WHERE id = #{userId}")
